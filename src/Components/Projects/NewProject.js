...
class NewProject extends Component {
    ...
    updateProjectInCache = (cache, { data: { updateProject } }) => {
        const query = ListProjectsQuery;
        const projectsData = cache.readQuery({ query });
        let projectList = [...projectsData.ProjectList];
        let updateIndex = projectList.findIndex(project => project.id === updateProject.id);
    ...
    render() {
        const mutationButton = this.state.action === 'ADD' ? (
            ...
            >
                {mutation => (
                    <button className="btn-primary btn" disabled={invalidInputs} onClick={() => this.mutationHandler(mutation)}>
                        Create
                    </button>
                )}
            </Mutation>
        ) : (
                <Mutation mutation={UpdateProjectQuery}
                    update={this.updateProjectsCache}
                    refetchQueries={[{ query: ListProjectsQuery }]}>
                    {mutation => (
                        ...
                        <form>
                            ...
                            <input type="text" id="defaultFormRegisterNameEx"
                                ...
                                onChange={event => {
                                    ...
                                    projectsData.ProjectList.forEach(project => {
                                        if (project.projectName.toLowerCase() === inputName.toLowerCase())
                                            found = true;
                                    });
                                    ...
                                    if (!this.state.nameConflict)
                                        this.setState({ projectName: inputName })
                                }} />
                            ...
                            <input type="text" id="defaultFormRegisterNameEx"
                                ...
                                onChange={event => {
                                    ...
                                    let found = false;
                                    projectsData.ProjectList.forEach(project => {
                                        if (project.projectKey.toLowerCase() === inputKey.toLowerCase())
                                            found = true;
                                    });
                                    ...
                                    if (!this.state.keyConflict)
                                        this.setState({ projectKey: inputKey })
                                }}
                                className="form-control"
                                maxLength="62" />
